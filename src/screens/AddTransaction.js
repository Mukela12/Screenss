...
const AddTransaction = () => {
  ...
  return (
    ...
    <TouchableOpacity className={`w-[55%] h-[80%] shadow border-[0.25px] border-[#614bc3] rounded-2xl flex items-center justify-center bg-[#614bc3] mb-4`}>
        <Text className={`font-bold text-[20px] text-white`}>{active === "expense" ? "Add Expense" : "Add Income"}</Text>
    </TouchableOpacity>

    <TouchableOpacity className={`w-[55%] h-[70%] shadow border-[0.20px] border-[#614bc3] rounded-2xl flex items-center justify-center bg-[#614bc3]`}>
        <Text className={`font-bold text-[20px] text-white`}>View Consumption</Text>
    </TouchableOpacity>
</View>
    </SafeAreaView>
  )
